      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            (entry.target as HTMLElement).style.animationDuration = `${animationDuration}s`;
  ...
  return (
    <section className="w-full flex  items-center  justify-center  py-24  ">
      <div className="w-full flex flex-col justify-center items-center ">
        ...
        <div ref={containerRef} className="grid items-center w-[90%] justify-center container grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
          {items.map((item, i) => (
            <Link
              key={i}
              href={item.path}
